/*
 * Generate public/og-banner.webp from scripts/og-banner.svg, with the exchange
...
// Count from the site's own content (the number of supported exchanges).
const content = readFileSync(resolve(root, 'index.md'), 'utf-8')
const m = content.match(/(\d+)\s+(?:crypto\s+)?exchanges/i)
if (!m) {
  console.error('gen-og-banner: could not find the exchange count in index.md')
  process.exit(1)
}
const count = m[1]

let svg = readFileSync(resolve(here, 'og-banner.svg'), 'utf-8')
svg = svg.replace(/\d+ exchanges/g, `${count} exchanges`)
...
console.log(`gen-og-banner: wrote public/og-banner.webp (${count} exchanges)`)
